...
window.addEventListener('mouseover', function (e) {
	...
	// isClass = e.target.classList;
	isClass = Array.from(e.target.classList);
	console.log('isClass = ', isClass);
	// for (let i = 0; i < isClass.length; i++) {
	// 	console.log(isClass[i]);
	// }
});
...
function mousedown(e) {
	let isHClass = Array.from(e.target.classList);
	console.log('ini adalah headfile : ', e.target.className);

	let posX = e.clientX;
	let posY = e.clientY;

	if (checkClass(isClass, isHClass)) {
		console.log('ini benarrr!!!!')
		//
	}
